...
local function run(msg, matches)
	if permissions(msg.from.id, msg.to.id, "lang_install") then
		...
		set_text(LANG, 'settings:20', '#arabic on/off: زبان عربي.')
		set_text(LANG, 'settings:21', '#english on/off: زبان انگليسي.')
		set_text(LANG, 'settings:22', '#emoji on/off: ارسال ايموجي.')
