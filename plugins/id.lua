...
function getID_by_reply_cb(arg, msg)
    send_msg(msg.chat_id_, lang_text(msg.to.id, 'userID') .. " \n" .. msg.sender_user_id_ .. lang_text(msg.to.id, 'chatID') .. " " .. msg.chat_id_, "md")
end

local function run(msg, matches)
    if not msg.reply_id then
    	send_msg(msg.to.id, lang_text(msg.to.id, 'userID') .. " \n" .. msg.from.id ..  lang_text(msg.to.id, 'chatID') .. " " .. msg.to.id, "md")
    else
